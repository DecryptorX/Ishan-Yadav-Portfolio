        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '3rem', marginBottom: '6rem', alignItems: 'start' }}>
          ...
          >
            <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem', lineHeight: 1.8, fontFamily: 'var(--font-sans)', fontWeight: 400, margin: 0 }}>
              A catalog of hardware configurations, workstation setups, development utilities, and editor plugins I use on a daily basis.
            </p>
          </motion.div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
          {SECTIONS.map((sec, idx) => (
            ...
            >
              ...
              {/* Title */}
              <h2 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#ffffff', marginBottom: '2.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', letterSpacing: '-0.02em', fontFamily: 'var(--font-display)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                {sec.items.map((item, ii) => (
                  <div key={ii} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', alignItems: 'flex-start', borderBottom: ii === sec.items.length - 1 ? 'none' : '1px solid rgba(255,255,255,0.03)', paddingBottom: ii === sec.items.length - 1 ? 0 : '1.5rem' }}>
                    <h3 style={{ fontSize: '0.94rem', fontWeight: 700, color: '#ffffff', margin: 0, fontFamily: 'var(--font-display)' }}>
                      {item.name}
                    </h3>
